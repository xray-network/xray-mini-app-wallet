    ...
    setTimeout(async () => {
      ...
      if (outputs.length && accountState && web3) {
        try {
          ...
          if (action === "send") {
            // setLoading(true) // TODO: create ping-pong loading state in xray-mini-app-sdk-react
    ...
    <div className="max-w-256 mx-auto pt-5">
      <div>
        <div className="flex items-center mb-5">
          ...
          <div className="ms-auto mb-0 flex items-center justify-center">
            ...
            <span
              ...
              onClick={resetForm}
            >
              <ArrowPathIcon className="size-4 me-1" strokeWidth={2} />
              Reset
            </span>
          </div>
        </div>
        <Form onFinish={() => processForm("send")} form={form} layout="vertical" requiredMark={false} preserve>
                ...
                {addressFields.map((addressField, index) => {
                  return (
                    <div key={addressField.key}>
                      <Form.Item>
                        <div className="flex items-center mb-2">
                          ...
                          {addressField.key > 0 && (
                            ...
                            >
                              <TrashIcon className="size-4 me-1" strokeWidth={2} />
                              <span>Remove</span>
                            </a>
                          )}
                        </div>
                                  ...
                                  >
                                    ...
                                    >
                                      {(accountAssets || []).map((asset) => {
                                        ...
                                        return (
                                          <Select.Option key={assetId} value={assetId} className={style.assetOption}>
                                            <div className={style.assetInfo}>
                                              <span className={style.assetIcon}>
                                                <AssetImage assetId={assetId} />
                                              </span>
                                              <span className={style.assetName}>
                                                <span>{Utils.decodeAssetName(asset.assetName)?.assetNameFinal}</span>
                                                <span>
                                                  {Utils.quantityFormat(asset.quantity, asset.decimals, true).final} —{" "}
                      ...
                      {index + 1 < addressFields.length && <div className="shared-line shared-line-dashed my-5" />}
                    </div>
                  )
                })}
              </>
          ...
          <div className="shared-line shared-line-dashed my-10" />
