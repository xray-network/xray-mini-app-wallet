...
| `C05`     | `IMPLEMENTED` | Split standalone preferences from bridge-owned state; npm-linked `xray-js`; consolidated app-local settings, provider, and transaction adapters under `app/integrations/xray-js`; replaced `cardano-web3-js`, the web3 store, CW3 terminology, and the local CRC-8 codec with its bridge hooks, network-driven Cardano provider, v4 transaction/address APIs, cardano-lib-backed `cip67` namespace, asset/hex helpers, configuration, and `CardanoTypes`; removed direct mini-app SDK and Buffer dependencies; separated routing effects; and moved Ant Design escape access into `theme`. | Exact sibling resolution, integration-tree/dependency/source scans, CIP-67 vector smoke, typecheck, production build, and legacy architecture/package/terminology/CRC scans passed. |
